...
function pctBar(pct: number | null) {
  ...
  return (
    <div className="flex items-center gap-2 mt-1">
      ...
      <span className="text-xs font-mono text-slate-500 w-12 text-right">{pct.toFixed(2)}%</span>
    </div>
  );
}
  ...
  return (
    <Layout>
      <div className="max-w-5xl mx-auto px-4 py-6">
        {/* Page title */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-slate-900">Investor Directory</h1>
        ...
        <div className="relative mb-5">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 text-sm border border-slate-200 rounded-lg bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm"
          />
          {query && (
            <button
              onClick={() => setQuery("")}
                  ...
                  <div className="flex items-center gap-4 px-4 py-3 rounded-lg border border-slate-200 bg-white hover:border-blue-200 hover:bg-blue-50/40 transition-colors duration-100 cursor-pointer">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold text-slate-900 truncate">{prop.name}</span>
                        {prop.isGrovePark && (
                      ...
                      <div className="text-xs text-slate-500 truncate mt-0.5">{prop.entity_name}</div>
                    </div>

                    {/* EIN */}
                    <div className="hidden sm:block text-xs font-mono text-slate-400 shrink-0">
                      EIN {prop.entity_ein}
                    </div>
